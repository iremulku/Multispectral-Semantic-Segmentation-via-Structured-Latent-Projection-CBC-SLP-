...
class MMVit5(nn.Module):
    def __init__(self, num_cls=1):
        super(MMVit5, self).__init__()
    ...
    def forward(self, x, mask=None):
        ...
        mask: [B, 3] veya None
              ...
              None → tüm modaliteler mevcut (orijinal davranış)
        """
        B = x.size(0)
        ...
        # --- 2) MaskModal benzeri feature-level maskeleme ---
        # mask yoksa hiçbir şey yapma
        if mask is not None:
            # mask: [B,3]
            rgb_mask  = mask[:, 0]
            nir_mask  = mask[:, 1]
        ...
        # (Opsiyonel) x6_intra seviyesinde de maskeyi tekrar uygulamak istersen:
        if mask is not None:
